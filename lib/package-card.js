...
export default class PackageCard {
      ...
      <div className='package-card col-lg-8'>
        ...
        <div className='meta'>
          <div ref='metaUserContainer' className='meta-user'>
            <a ref='avatarLink' href={`https://atom.io/users/${owner}`}>
            ...
            <a ref='loginLink' className='author' href={`https://atom.io/users/${owner}`}>{owner}</a>
          </div>
  ...
  handleButtonEvents (options) {
    ...
    const packageNameClickHandler = (event) => {
      event.stopPropagation()
      shell.openExternal((this.pack.repository && this.pack.repository.url) || `https://github.com/atomeditor-io/${this.pack.name}`)
    }
